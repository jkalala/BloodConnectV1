...
-- This script fixes the column name conflicts and adds missing columns

-- First, let's check if the old donor_responses table exists and drop it
DROP TABLE IF EXISTS donor_responses CASCADE;

-- Create the new donor_responses table with correct structure
...
CREATE POLICY "Users can view responses to their requests" ON donor_responses
  FOR SELECT USING (
    EXISTS (
      ...
      AND blood_requests.contact_phone = (SELECT phone FROM users WHERE id = auth.uid())
    )
  );
