...
describe('User Registration Schema', () => {
  ...
  it('should reject invalid phone numbers', () => {
    ...
    expect(result.errors).toContain('phone: Phone number must be in international format (+1234567890)')
  })
  ...
  it('should reject names with invalid characters', () => {
    ...
    const result = validateInput(userRegistrationSchema, invalidData)
    expect(result.success).toBe(false)
    expect(result.errors).toContain('name: Name contains invalid characters')
  })
...
describe('Blood Request Schema', () => {
  ...
  it('should reject invalid units needed', () => {
    ...
    expect(result.success).toBe(false)
    expect(result.errors).toContain('units_needed: At least 1 unit is required')
  })
  ...
  it('should reject too many units', () => {
    ...
    const result = validateInput(bloodRequestSchema, invalidData)
    expect(result.success).toBe(false)
    expect(result.errors).toContain('units_needed: Cannot request more than 20 units at once')
  })
  ...
  it('should reject past expiry dates', () => {
    ...
    const result = validateInput(bloodRequestSchema, invalidData)
    expect(result.success).toBe(false)
    expect(result.errors).toContain('expires_at: Expiry date must be in the future')
  })
})
...
describe('Verification Code Schemas', () => {
  ...
  it('should reject invalid verification codes', () => {
    ...
    expect(result.success).toBe(false)
    expect(result.errors).toContain('code: Verification code must be 6 digits')
  })
...
describe('User Profile Update Schema', () => {
  ...
  it('should reject too long medical conditions', () => {
    ...
    expect(result.success).toBe(false)
    expect(result.errors).toContain('medical_conditions: Medical conditions must not exceed 1000 characters')
  })
})