...
export interface WhatsAppMessage {
  ...
  template?: any        // Add template as optional
  language?: string
}
...
export class WhatsAppBusinessService {
   ...
   */
  async sendBloodRequestNotification(phoneNumber: string, requestData: any): Promise<boolean> {
    try {
      const message = {
        to: phoneNumber,
   ...
   */
  async sendEmergencyAlert(phoneNumber: string, alertData: any): Promise<boolean> {
    try {
      const message = {
        to: phoneNumber,
        type: 'interactive' as const,
   ...
   */
  async sendDonorMatchNotification(phoneNumber: string, matchData: any): Promise<boolean> {
    try {
      const message = {
        to: phoneNumber,
        type: 'interactive' as const,
   ...
   */
  async sendDonationReminder(phoneNumber: string, reminderData: any): Promise<boolean> {
    try {
      const message = {
        to: phoneNumber,
   ...
   */
  async sendWelcomeMessage(phoneNumber: string, userData: any): Promise<boolean> {
    try {
      const message = {
        to: phoneNumber,
        type: 'interactive' as const,
   ...
   */
  async sendStatusUpdate(phoneNumber: string, statusData: any): Promise<boolean> {
    try {
      const statusEmoji = {
        'pending': '⏳',
        'in_progress': '🔄',
   ...
   */
  async sendLocationBasedRequest(phoneNumber: string, requestData: any): Promise<boolean> {
    try {
      const message = {
        to: phoneNumber,
        type: 'interactive' as const,
   ...
   */
  private async handleInteractiveMessage(phoneNumber: string, interactive: any): Promise<void> {
