...
export class MobileMoneyService {
  ...
  async getTransactionStats(): Promise<any> {
    try {
      ...
      const totalAmount = stats?.reduce((sum: number, tx: any) => sum + tx.amount, 0) || 0
      const totalTransactions = stats?.length || 0
      const donationRewards = stats?.filter((tx: any) => tx.type === 'donation').length || 0
      const emergencyPayments = stats?.filter((tx: any) => tx.type === 'emergency').length || 0
   ...
   */
  private async processPaymentWithProvider(provider: MobileMoneyProvider, transaction: any): Promise<{ success: boolean; message: string }> {
    try {
   ...
   */
  private async processMpesaPayment(provider: MobileMoneyProvider, transaction: any): Promise<{ success: boolean; message: string }> {
    try {
   ...
   */
  private async processAirtelMoneyPayment(provider: MobileMoneyProvider, transaction: any): Promise<{ success: boolean; message: string }> {
    try {
   ...
   */
  private async processMTNMoMoPayment(provider: MobileMoneyProvider, transaction: any): Promise<{ success: boolean; message: string }> {
    try {
   ...
   */
  private async processOrangeMoneyPayment(provider: MobileMoneyProvider, transaction: any): Promise<{ success: boolean; message: string }> {
    try {
   ...
   */
  async handlePaymentCallback(provider: string, callbackData: any): Promise<void> {
    try {
