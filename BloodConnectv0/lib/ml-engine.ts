...
export class MLEngine {
   ...
   */
  private calculateFeatures(donor: any, request: any): DonorFeatures {
    const now = new Date()
   ...
   */
  private async calculateHistoricalFeatures(donor: any, request: any, response: any): Promise<DonorFeatures | null> {
  ...
  private calculateRecentActivity(donor: any): number {
  ...
  private generateExplanation(features: DonorFeatures, importance: { feature: string; weight: number }[]): string {
    ...
    for (const factor of topFactors) {
      const featureValue = (features as any)[factor.feature]
